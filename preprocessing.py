...
#%% groupby playlist --> clustering_matrix
"""
...
total_word = matrix.get_cluster_matrix( merged_t.tag_gnr_title(id_title = tokenized_title) , word_vec , by = 'word')
...
total_singer = matrix.get_cluster_matrix( merged_t.title_singer(id_title = tokenized_title) , singer_vec , by = 'word')
...
# tags
tag_df = pd.read_pickle('data/preprocessed/tags_df.pickle') # clustring_tag 함수를 실행시키면 자동으로 저장됨
...
# genre
total_gnr = matrix.get_genre_matrix()
...
train_gnr.to_csv("data/matrix/train/train_gnr.csv" , index = False)


# album
album_df = pd.read_pickle('data/preprocessed/album_df.pickle')
album_vec = pd.read_pickle('data/digitization/clu_album_emb100.pickle')
